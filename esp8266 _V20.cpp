...
void read(){
  ...
  delay (5);
  node.getResponseBuffer(temp3);
  
  node.readHoldingRegisters(200, 1);
  delay (5);
  node.getResponseBuffer(temp4);
  
  node.readHoldingRegisters(259, 1);
  delay (5);
  node.getResponseBuffer(temp5);
  
  node.readHoldingRegisters(260, 1);
  delay (5);
  node.getResponseBuffer(temp6);
  
  node.readHoldingRegisters(219, 1);
  delay (5);
  node.getResponseBuffer(temp7);
...
void loop() {
  ...
  if (temp == node.ku8MBSuccess) {//如果通信成功
    ...
    Serial.print("] ");
    Serial.print("ERROR:");
    Serial.println(temp, HEX );//以HEX（16）进制显示错误信息（E2：你的TX,RX断线，E0：你的485转ttl板子有问题）
